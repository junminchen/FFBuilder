...
def debug_bf4_v2():
    ...
    # Fitting with Symmetry: 4 F atoms must be same
    # Variables: x[0] = B charge, x[1] = F charge
    def objective(x):
        ...
        # Hyperbolic restraint (Stronger: a=0.001)
        a = 0.001; b = 0.1
        restraint = a * (np.sqrt(x[0]**2 + b**2) - b) + 4 * a * (np.sqrt(x[1]**2 + b**2) - b)
        return chi_sq + restraint

    cons = ({'type': 'eq', 'fun': lambda x: x[0] + 4*x[1] - total_charge})
    res = minimize(objective, [0.1, -0.275], constraints=cons, method='SLSQP')
    
    print("\n--- Final Symmetrical RESP Charges ---")
